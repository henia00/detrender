...
class MatplotlibTkinterApp:
    ...
    def create_widgets(self):
        ...
        self.button_poly_subtract = tk.Button(self.root, text="Subtract polynomial", command=self.subtract_poly)
        self.button_poly_subtract.pack(side=tk.LEFT)

        self.button_remove_individual = tk.Checkbutton(self.root, text="Remove individual points", variable=self.individual_points_var, command=self.update_checkbuttons)        
        self.button_remove_individual.pack(side=tk.LEFT)

        self.button_remove_individual = tk.Checkbutton(self.root, text="Remove x range", variable=self.x_range_var, command=self.update_checkbuttons)        
        self.button_remove_individual.pack(side=tk.LEFT)

        self.button_restore = tk.Button(self.root, text="Restore original", command=self.restore_original_data)
    ...
    def load_data(self):
        ...
        self.removed = []
        file_path = filedialog.askopenfilename(title="Select a file", filetypes=[("Dat files", "*.dat"), ("Text files", "*.txt"), ("All files", "*.*")])
        self.file_name = os.path.basename(file_path)
        if file_path:
    ...
    def restore_original_data(self):
        ...
        self.x_normalized = normalize(self.data[:, 0], self.ax.get_xlim()[0], self.ax.get_xlim()[1])
        self.y_normalized = normalize(self.data[:, 1], self.ax.get_ylim()[0], self.ax.get_ylim()[1])
    ...
    def save_files(self):
        #np.savetxt(self.file_name+"_polynomial.out", self.fitted_poly_coefficients, delimiter=' ', fmt='%f')
        np.savetxt(os.path.join(self.out_path,self.file_name)+"_data", self.data, delimiter=' ', fmt='%f')
        if len(self.removed) > 0:
            np.savetxt(os.path.join(self.out_path,self.file_name)+"_removed", self.removed, delimiter=' ', fmt='%f')
        if hasattr(self, 'fitted_poly_line'):
            print("zapisywany polynomial ")
            print(self.fitted_poly_coefficients)
            polynomial_order = int(self.poly_degree_entry.get())
            header_text=f"Polynomial coefficients and the first element corresponds to the coefficient of the highest-degree term \n Polynomial order: {polynomial_order}"
            np.savetxt(os.path.join(self.out_path,self.file_name) + "_polynomial", self.fitted_poly_coefficients, header=header_text, comments='#', delimiter=' ')#, fmt='%f')
        exit()
    ...
    def fit_poly(self):
        ...
        #removed_indices = ~np.all(np.isin(self.data, self.removed), axis=1)
        #remaining_data = self.data[removed_indices]

       
        # Check if there are enough remaining points to fit a polynomial
        if len(self.data) < int(self.poly_degree_entry.get()) + 1:
            print("Not enough remaining points to fit a polynomial.")
            return

        # Fit the polynomial to the remaining points
        ...
        self.fitted_poly_line, = self.ax.plot(x_values, y_values, label=f'Fitted Polynomial (Degree {degree})', color='green') 
        print(self.fitted_poly_coefficients)
        # Redraw the canvas
        self.canvas.draw()
    ...
    def subtract_poly(self):
        ...
        # Convert self.removed to a NumPy array if it's not already
        #removed_array = np.array(self.removed)
    ...
    def remove_point(self, clicked):
        ...
        closest_point_index = np.argmin(np.sqrt((self.x_normalized - x_clicked)**2 + (self.y_normalized - y_clicked)**2))
        closest_point = self.data[closest_point_index]
        #print(closest_point)

        
        
        #self.removed.append(closest_point)
